...
function renderWorkGrid(container, items, append) {
  if (!container) return;
  if (!append) container.innerHTML = "";

  if (!Array.isArray(items) || items.length === 0) {
    if (!append) container.innerHTML = `<p class="muted">No items yet.</p>`;
    return;
  }
...
function renderUpcoming(container, items) {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    container.innerHTML = `<p class="muted">Nothing scheduled right now.</p>`;
    return;
  }
  ...
  for (const item of items) {
    ...
    const titleHtml = firstLink
      ? `<span class="upcoming-title"><a href="${escapeAttr(firstLink.url)}" target="_blank" rel="noopener">${title}</a></span>`
      : `<span class="upcoming-title">${title}</span>`;

    const meta = Array.isArray(item.meta) ? item.meta : [];
    const venueMeta = meta.find(m => m.label === "Venue" || m.label === "Type");
    const detailParts = [];
    if (venueMeta) detailParts.push(escapeHtml(venueMeta.value));
    if (text) detailParts.push(text);
    const detailHtml = detailParts.length
      ? `<span class="upcoming-detail">— ${detailParts.join(". ")}</span>`
      : "";

    row.innerHTML = `
      ${titleHtml}
      ${detailHtml}
      ${when ? `<span class="upcoming-when">${when}</span>` : ""}
    `;
...
async function main() {
  ...
  // Render embeds + work cards into a single unified grid
  const workGrid = safeGetEl("workGrid");
  if (workGrid) {
    workGrid.innerHTML = "";

    // 1. Instagram embeds first
    const embeds = content?.work?.instagramEmbedsHtml || [];
    ...
    // 2. Then work cards
    renderWorkGrid(workGrid, content?.work?.items || [], /* append */ true);

    // 3. Process Instagram embeds
    await ensureInstagramScriptLoaded();
    if (window.instgrm?.Embeds?.process) {
      window.instgrm.Embeds.process();
    }
  ...
  // Upcoming
  setText("upcomingHeading", content?.upcoming?.heading || "Upcoming");
  renderUpcoming(safeGetEl("upcomingGrid"), content?.upcoming?.items || []);
